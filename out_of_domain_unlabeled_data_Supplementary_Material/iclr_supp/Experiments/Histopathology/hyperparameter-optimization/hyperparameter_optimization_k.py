...
argParser.add_argument("-n", "--hpnum", help="Hyper parameter optimization experiment number/name")
...
class EmbeddingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        embedding_vector = torch.tensor(self.embeddings[idx],device=device)
        label_onehot = torch.tensor(self.labels[idx],device=device)
...
def run_model(config=None,num_classes=10):
    ...
    with wandb.init(entity='kartikn_', project=proj_name, name=f"{hp_num}_{ex_num}", 
        ...
        y_train, dataloaders_labeled, dataloaders_unlabeled, dataset_sizes = load_data(myconfig, X_train, y_train, X_val, y_val, X_data_test, y_data_test, X_data_ul, has_unlabeled=has_unlabeled)
...
total_ex_num = 50
import time
hp_num = f"{args.hpnum}-L{args.labeled_number}_UL{args.unlabeled_number}_{args.num_classes}_{time.time()}"
