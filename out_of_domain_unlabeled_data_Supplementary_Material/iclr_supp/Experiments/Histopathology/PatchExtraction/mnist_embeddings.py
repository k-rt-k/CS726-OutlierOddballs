...
eval_transforms = transforms.Compose([
    ...
    transforms.Grayscale(num_output_channels=3),  # Convert images to 3 channels (for ResNet)
    transforms.ToTensor(),
...
def main(data_path, output_path, args):
    ...
    dataset = datasets.MNIST(root=dataroot, train=not args.test, download=True, transform=eval_transforms)
    ...
    data_path = os.path.join(output_path, "mnist_{}_embeddings.pkl".format("train" if not args.test else "test"))
